...
Ключевые файлы: `server/internal/store/teacher_lessons.go`,
`server/internal/api/teacher_lesson_handlers.go`, `web/src/pages/Teachers.tsx`,
`web/src/pages/LessonEditor.tsx`, `web/src/pages/AdminLessons.tsx`,
`frontend/lib/screens/community_lessons_screen.dart`.

Публичный урок адресуется стабильным `slug`, закрытый — случайным
`share_token`. Содержимое всегда берётся из `published_revision_id`; правка
создаёт новую ревизию и не меняет уже опубликованный JSON. Публичная ревизия
переходит `draft -> pending -> published|rejected`, а unlisted может перейти в
`published` без второй модерации, если аккаунт преподавателя уже одобрен.

Веб — единственный редактор. Flutter читает каталог и уроки, кеширует успешно
открытые документы и позволяет подать/проверить статус заявки. Видео на web
встраивается через нормализованный embed, на native открывается исходная
HTTPS-ссылка в системном приложении провайдера.

Публичный player, предпросмотр редактора и предпросмотр модератора используют
один `LessonPlayer`: сначала показывается теория, затем упражнения по одному,
после них диалог и завершение. Администратор открывает очередь на
`/admin/lessons`; одобрение pending-ревизии сразу делает её публичной. Flutter
повторяет этот порядок и получает опубликованные уроки из того же `/v1/lessons`,
поэтому каталог обновляется независимо от версии установленного клиента.

> Актуальность документа: 2 августа 2026 года.
...
- `ReaderSettings.flow` выбирает `PageView.builder` или непрерывный
  `ScrollablePositionedList`; оба строят только видимое окно;
...
- в непрерывном режиме системная прокрутка не перехватывается, а видимая
  логическая страница определяется через `ItemPositionsListener`;
...
- режим чтения: страницы или непрерывная вертикальная прокрутка.
