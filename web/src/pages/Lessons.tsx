...
function LessonTile({lesson}:{lesson:Lesson}){return <Link to={`/lessons/${lesson.slug}`} className="group rounded-lg border border-[var(--line)] bg-[var(--bg-raised)] p-5 transition-colors hover:border-[var(--accent)]"><div className="flex items-center justify-between gap-3"><span className="rounded-md bg-[var(--accent)]/10 px-2 py-1 text-xs font-bold text-[var(--accent)]">{lesson.level}</span><span className="text-xs text-[var(--text-muted)]">{label(lesson.lessonType)}</span></div><h2 className="mt-4 text-xl group-hover:text-[var(--accent)]">{lesson.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-[var(--text-muted)]">{lesson.summary}</p><div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-xs text-[var(--text-muted)]"><span className="inline-flex items-center gap-1"><LuClock3/>{lesson.estimatedMinutes} мин</span><span className="inline-flex items-center gap-1"><LuGraduationCap/>{lesson.authorName}</span></div></Link>}
