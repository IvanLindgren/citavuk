    ...
    <><ProfileEditor/><section className="mt-10 border-t border-[var(--line)] pt-8">
      ...
      {lessons.length === 0 ? (
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {lessons.map((lesson) => <Link key={lesson.id} to={`/teachers/lessons/${lesson.id}`} className="rounded-lg border border-[var(--line)] bg-[var(--bg-raised)] p-5 transition-colors hover:border-[var(--accent)]"><div className="flex justify-between gap-3"><span className="text-xs font-bold uppercase text-[var(--accent)]">{lesson.level} · {typeLabel(lesson.lessonType)}</span><span className="text-xs text-[var(--text-muted)]">{statusLabel(lesson)}</span></div><h3 className="mt-3 text-lg">{lesson.title}</h3><p className="mt-2 line-clamp-2 text-sm text-[var(--text-muted)]">{lesson.summary}</p></Link>)}
        </div>
      )}
