...
function GrammarPanel({ analysis }: { analysis: WordAnalysis }) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--line)] bg-[var(--bg-sunken)] p-4">
      ...
      {/* Слова нет в словаре форм, и начальную форму подсказала нейросеть.
          Падеж и таблицы посчитаны по правилам — но читатель должен знать,
          что за этим разбором не стоит словарная статья. */}
      {analysis.generated && (
        <p className="mt-3 border-t border-[var(--line)] pt-2 text-xs italic leading-relaxed text-[var(--text-muted)]">
          Этого слова нет в словаре Читавука: начальную форму подсказала
          нейросеть, а падеж и склонение построены по правилам языка.
        </p>
      )}
    </div>
  );
