...
export function withPin(
  ...
  if (pin && pin.word.trim()) {
    pins[objectId] = { ...pin, word: pin.word.trim() };
  } else {
...
/**
 * Порядок обхода дворца.
 *
 * Обход всегда идёт в одном и том же порядке — том, в котором предметы описаны
 * в сцене. В этом весь приём: маршрут должен быть один и тот же, иначе место
 * перестаёт быть подсказкой. Перемешивать карточки здесь было бы ошибкой,
 * хотя для обычного повторения это норма.
 */
export function walkOrder(
  ...
  return objectIds
    ...
    .map((id) => ({ objectId: id, pin: palace.pins[id]! }));
}
