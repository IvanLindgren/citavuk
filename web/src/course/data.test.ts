...
describe("course answer rules", () => {
  it("выбор старта пропускает предыдущие темы без фиктивных результатов", () => {
    const bundle = courseBundle();
    const lessons = bundle.units[0]!.skills[0]!.lessons;
    const template = lessons[0]!;
    lessons.push(
      { ...template, id: "second", prerequisites: ["lesson"] },
      { ...template, id: "third", prerequisites: ["second"] },
    );
    const p = startCourseFrom(bundle, "third");
    expect(lessonUnlocked(lessons[2]!, p)).toBe(true);
    expect(p.lessons.lesson?.skipped).toBe(true);
    expect(p.lessons.lesson?.status).toBe("available");
    expect(p.lessons.lesson?.completedAt).toBeNull();
    expect(p.xp).toBe(0);
    expect(p.streak.currentDays).toBe(0);
    expect(p.mastery).toEqual({});
    expect(loadProgress(bundle).lessons.second?.skipped).toBe(true);
    const earned = saveLessonProgress(bundle, "lesson", 1);
    const merged = mergeProgress(p, earned, bundle);
    expect(merged.lessons.lesson?.skipped).toBe(false);
    expect(merged.lessons.lesson?.bestScore).toBe(1);
    expect(startCourseFrom(bundle, "second").lessons.second?.skipped).toBe(
      false,
    );
    expect(() => startCourseFrom(bundle, "missing")).toThrow();
    lessons[0]!.prerequisites=['third'];
    const direct=startCourseFrom(bundle,'lesson');
    expect(lessonUnlocked(lessons[0]!,direct)).toBe(true);
  });
  ...
  it("listening_qa is scored as a whole and only when every question is answered", () => {
    ...
    expect(canEvaluate(exercise, { kind: "pairs", values: { q1: "a" } })).toBe(
      false,
    );
    expect(
    ...
    expect(
      evaluate(exercise, { kind: "pairs", values: { q1: "a", q2: "c" } })
        .correct,
    ).toBe(true);
    expect(
      evaluate(exercise, { kind: "pairs", values: { q1: "a", q2: "d" } })
        .correct,
    ).toBe(false);
  });
  ...
  it("checks sentence order rather than the set of selected words", () => {
    ...
    expect(evaluate(exercise, { kind: "order", ids: ["a", "b"] }).correct).toBe(
      true,
    );
    expect(evaluate(exercise, { kind: "order", ids: ["b", "a"] }).correct).toBe(
      false,
    );
  });
  ...
  it("randomizes exercises inside each difficulty stage", () => {
    ...
    expect(
      randomizeExercises(exercises, () => 0).map((item) => item.id),
    ).toEqual(["easy-b", "easy-a", "hard-b", "hard-a"]);
    expect(
