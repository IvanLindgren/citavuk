...
//
// Планируемые разделы пропускаются. Сейчас все четыре раздела активны, но флаг
// остаётся частью контракта: новый раздел можно будет показать заранее, не
// блокируя им переход. Пустая ступень не открывается сама собой.
