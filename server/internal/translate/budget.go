...
// DefaultRunesPerDay — суточная доля месячной квоты. Делим на 31, а не на 30:
// в короткий месяц лучше недобрать, чем упереться в квоту за день до её
// обновления.
//
// Доля вдвое больше месячной квоты free-плана — это решение, а не описка:
// расходовать разрешено быстрее, чем квота восстанавливается. Упрётся раньше
// срока — крутить CITAVUK_DEEPL_RUNES_PER_DAY на сервере, без пересборки.
const DefaultRunesPerDay = 1_000_000 / 31
