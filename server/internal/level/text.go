...
func Estimate(lex *lexicon.Lexicon, paragraphs []string) TextLevel {
	...
	// одна — текст сложнее любой ступени шкалы, то есть C1. Покрытие тогда
	// показывается по последней ступени: доля «в пределах бесконечности» равна
	// единице всегда и не значит ничего.
	result.Level = store.SerbianLevels[len(store.SerbianLevels)-1]
