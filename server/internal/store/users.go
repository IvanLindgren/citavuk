...
// disposableEmailDomains — домены одноразовой почты. Регистрация с такого
// адреса бессмысленна: человек не сможет ни восстановить пароль, ни прочитать
// письмо завтра, а сервис тратит квоту отправки на заведомо брошенные ящики.
...
func IsDisposableEmail(email string) bool {
	...
	// Локальная часть тоже может содержать «@» в кавычках, но до такого адреса
	// ValidateEmail всё равно не дойдёт — берём домен после последнего «@».
	if i := strings.LastIndex(domain, "@"); i >= 0 {
		domain = domain[i+1:]
	}
	_, found := disposableEmailDomains[strings.ToLower(strings.TrimSpace(domain))]
	return found
}
