  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        Text(
          'Когда начинаешь учить язык, а особенно такой редкий, как сербский, '
          'рано или поздно упираешься в вопрос: что делать после пары-тройки '
          'грамматических упражнений и быстрых онлайн-курсов? Откуда брать '
          'новые слова, что смотреть? Здесь я, разработчик Читавука, '
          'постарался разложить слова, темы и упражнения сербского по уровням '
          'CEFR и по знакомым из английского категориям: Reading (Čitanje), '
          'Grammar (Gramatika), Vocabulary (Vokabular), Writing (Pisanje).\n\n'
          'Категории Speaking здесь нет намеренно: при всей её важности через '
          'интернет она не развивается. Даже распознавание речи с голосовым '
          'чат-ботом выглядело бы искусственно — живого собеседника оно не '
          'заменит. Тут остаётся только искать носителей или преподавателей и '
          'говорить с ними.',
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Выбирай (или просто смотри) любой уровень, к которому сейчас '
          'стремишься. Дальше можно открыть любой раздел этого уровня '
          'и увидеть:',
          style: theme.textTheme.bodyMedium?.copyWith(height: 1.5),
        ),
        const SizedBox(height: 10),
        ...
        const _Bullet(
          'Vocabulary — список слов по категориям (животные, дом, семья и так '
          'далее); их можно добавлять в свой словарь и отмечать выученные.',
        ),
        const _Bullet(
          'Writing — упражнения на написание предложений и игра '
          '«Ты против переводчика».',
        ),
        const SizedBox(height: 10),
        Text(
          'Читавук сам отслеживает прогресс и показывает его в личном кабинете '
          'по каждому разделу уровня. Набрал больше 80% во всех разделах — '
          'можно переходить на следующий уровень. Если цель не выбрана, ходить '
          'между разделами любого уровня можно свободно.',
          style: theme.textTheme.bodyMedium?.copyWith(height: 1.5),
        ),
        const SizedBox(height: 10),
        Text(
